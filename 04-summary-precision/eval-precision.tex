...
\begin{figure}
	...
	\resizebox{.8\textwidth}{!}{
		\input{04-summary-precision/figures/single-type-summary-size}
	}
...
%we note that the compression ratio of a  is higher than with the types summary $\mathcal{S}_t$.% The reason is that a node of the entity graph can be mapped to several sumnodes with the \emph{Single-Type} summary $R_{st}$, which leads to an increase of the size of the summary.
