...
We compare in this paragraph the performance of both ranking functions when applied over the Types summary of DBpedia. First, we evaluate the impact of the normalisation parameters on the \gls{MF}-based ranking approach. Then, we discuss the ranking performance of the basic and \gls{MF}-based approaches.
...
Table~\ref{tab:summary-ranking-cmp} reports the MAP values of the basic and \gls{MF}-based approaches over the Types summary of Dbpedia. We write the MAP value in bold for the approach that has the greatest. We observe that for a majority of the query complexities, the \gls{MF}-based approach provides a better ranking than the basic approach.
