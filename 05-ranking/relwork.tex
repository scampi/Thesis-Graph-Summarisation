...
We consider an entity to be a graph centered around a node, that we call the root, where each edge provides some description of that node. We define a rooted entity as the \emph{entity description} (Section~\ref{sec:entity-description}) of a \emph{root} node, where every node in the entity description is reachable from that root node.

\begin{definition}[Rooted Entity]
Let $G = \left\langle V, A, l_V \right\rangle$ be a graph, and let $\gls{edesc}(u \in V) \subseteq A$ be the entity description of the node $u$. We define the entity rooted in $u$ as the entity description $\gls{edesc}(u)$ such that there is a path from $u$ to every node in $\gls{edesc}(u)$.
\end{definition}
...
\begin{description}
	\item[term:] a node label can be split over some character, e.g., a white space; a split is then called a term.
...
\begin{labeling}{\textit{\underline{Step 1:}}}
	\item[\textit{\underline{Step 1:}}] In a first step, each field of an entity is assigned value that reflects its relevance with regards to a term in the query.
...
The first step consist in normalising the term frequency with regards to the length of a field in the entity, compared to its average in the dataset.
...
\begin{itemize}
	...
	\item $b_a$ is the normalization parameter for the field $a$ with $b_a \in \left[0,1\right]$;
...
\begin{equation}
tfn = \frac{\sum_{a \in e}{\bar{f}_{t,e,a}}\times(k_1+1)}{\sum_{a \in e}{\bar{f}_{t,e,a}}+k_1}
\label{eq:bm25f_2}
\end{equation}
...
\begin{enumerate}
	...
	\item the term frequency normalization model.
\end{enumerate}
...
In the first step, the term frequency normalization model used is the\emph{normalization 2F}. Given a query term $t$ in the entity $e$, this model is a per-field normalisation of the term frequency and is defined as follows:
\begin{equation}
...
In the second step, we apply the other two components of PL2F over the sum $tfn$ of the normalised term frequency across fields, i.e., $tfn = \sum_{a \in e}{\bar{f}_{t,e,a}}$:
