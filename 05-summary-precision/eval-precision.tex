...
In this section, we evaluate the trade-off between the performance of an implementation of a \gls{summarisation-relation}, the compression ratio, and the precision of the graph summary.
...
In conclusion, we observe that a combination of both type and attribute features is necessary to achieve a good precision. The results show that taking incoming attributes as a feature of the \gls{summarisation-relation} is important for the connectivity precision, but not for the schema.
