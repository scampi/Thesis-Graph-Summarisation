...
\begin{quote}
	...
	A deeper insight is achieved with the schema on Figure~\ref{fig:sg2} generated by grouping entities based on their type feature, e.g., we can see that a Person is connected to a City via the link ``lives''.
\end{quote}
...
In this section, we present a RDF model for describing a graph summary. This allows the summary $\mathcal{G}$ to be queried as any other RDF graph, ultimately to be used in conjunction with the entity graph $G$.

Viewing the graph summary as a RDF graph requires the creation of URIs for identifying sumnodes and sumedges. A URI for a sumnode can be created based on the features the summarisation relation is based on, e.g., on the type information for the \emph{Types} summary $R_t$. The sumedge $(x, \alpha, y) \in \mathcal{A}$ URI can be created by considering the source $x$ and target $y$ sumnodes, as well as the attribute $\alpha$.
...
\begin{figure}
	...
	\begin{subfigure}[b]{.45\textwidth}
		\centering
		\resizebox{\textwidth}{!}{
			\input{04-summary/figures/rdf-summary-node}
		}
		\caption{Node class}
		\label{fig:rdf-node}
	\end{subfigure}
	...
	\begin{subfigure}[b]{.35\textwidth}
		\centering
		\resizebox{\textwidth}{!}{
			\input{04-summary/figures/rdf-summary-edge}
		}
		\caption{Edge class}
		\label{fig:rdf-edge}
	\end{subfigure}
	...
	\begin{subfigure}[b]{\textwidth}
		\resizebox{\textwidth}{!}{
			\begin{tabular}{lc@{\hs}l}
				...
				SEdge & \phantom{a} & This class refers to a sumedge $(x, \alpha, y) \in \mathcal{A}$ of the summary. \\
				SNode & \phantom{a} & This class refers to a sumnode $x \in \mathcal{V}$ of the summary. \\
				\midrule
				...
				source & \phantom{a} & The sumnode $x$ in the sumedge $(x, \alpha, y) \in \mathcal{A}$. \\
				statistic & \phantom{a} & A statistic associated with a sumnode or a sumedge, e.g., \emph{count}. \\
				target & \phantom{a} & The sumnode $y$ in the sumedge $(x, \alpha, y) \in \mathcal{A}$. \\
...
\begin{table}
	...
	\begin{subfigure}{.475\textwidth}
		...
		\resizebox{\textwidth}{!}{
			\begin{tabular}{lllc@{\s}lll}
				\toprule
				\multicolumn{3}{c}{$G$} & \phantom{a} & \multicolumn{3}{c}{$\mathcal{G}_t$} \\
	...
	\end{subfigure}
	\caption{Full and lite reification of a \emph{Types} summary $R_t$. The summarisation relation $R_t$ has with the example data a one-to-one mapping from the graph $G$ to the summary $\mathcal{G}_t$. The node \emph{p1} is mapped to the sumnode \emph{t1} and \emph{d1} to \emph{t2}.}
