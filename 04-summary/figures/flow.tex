...
\node[draw,circle,right of = parse] (edesc) {$\gls{edesc}(u)$};
...
\node[draw,circle,right of = edesc] (gsv) {$\mathcal{V}$};
...
\node[draw,circle,right of = joinobj] (relagg) {$\mathcal{A}$};
\node [right of = joinobj,label={[label distance=.1cm]10:\scriptsize{GB}}] {};

\node[draw,circle,right of = gsv] (attagg) {$\mathcal{A}$};
...
\node[right of = attrdf,draw,regular polygon, regular polygon sides=3,shape border rotate=-90] (attout) {$\mathcal{G}$};
\node[right of = relrdf,draw,regular polygon, regular polygon sides=3,shape border rotate=-90] (relout) {$\mathcal{G}$};
