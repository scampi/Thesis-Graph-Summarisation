...
\begin{definition}[Source Material]
Let $G=\left\langle V, A, l_V \right\rangle$ be a graph. We call source material the set $\mathcal{Z}$ such that $\mathcal{Z} \cap V = \emptyset$.
\end{definition}
...
We map such node of the graph to a sumnode that we say to be \emph{undefined} for the summarisation relation, and we label it as $\mathfrak{U}$.
