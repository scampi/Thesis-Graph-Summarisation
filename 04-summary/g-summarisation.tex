...
We denote as \emph{feature} elements of the graph on which the \gls{summarisation-relation} is based on. As an example, attributes is a feature of the \gls{attributes-summary} as defined in Definition~(\ref{def:a}).
...
Let $G=\left\langle V, A, l_V \right\rangle$ and $\glssymbol{fbt-bisimulation-summary} = \left\langle \mathcal{W}_{fbt}, \mathcal{B}_{fbt}, l_{\mathcal{W}_{fbt}} \right\rangle$ be two graphs. Let $\approx_t \subseteq V \times V$ be a fbt-bisimulation relation.
...
The nodes $v_3$, $v_4$, and $v_5$ are $\sim_a$-equivalent because they share the same set of attributes, i.e., $\left\lbrace label, location, type \right\rbrace$. The graph in Figure~\ref{fig:attributes-summary} is the \gls{attributes-summary} of the graph in Figure~\ref{fig:graph}.
...
The relation $R_{ioat}$ maps a node based on the set of \gls{types}, and the incoming and outgoing attributes sets. We call the summary this relation generates the \emph{IO attributes \& types} summary.
...
\begin{table}
	...
	\emph{Unique Type} & \phantom{a} & $\glssymbol{unique-type-summary} = \left\langle \mathcal{W}_{ut}, \mathcal{B}_{ut}, l_{\mathcal{W}_{ut}} \right\rangle$ & \phantom{a} & Definition (\ref{def:st}) \\

	\emph{Types} & \phantom{a} & $\glssymbol{typessummary} = \left\langle \mathcal{W}_t, \mathcal{B}_t, l_{\mathcal{W}_t} \right\rangle$ & \phantom{a} & Definition (\ref{def:t}) \\

	\emph{Attributes} & \phantom{a} & $\glssymbol{attributes-summary} = \left\langle \mathcal{W}_a, \mathcal{B}_a, l_{\mathcal{W}_a} \right\rangle$ & \phantom{a} & Definition (\ref{def:a}) \\

	\emph{Attributes \& Types} & \phantom{a} & $\glssymbol{attributes-types-summary} = \left\langle \mathcal{W}_{at}, \mathcal{B}_{at}, l_{\mathcal{W}_{at}} \right\rangle$ & \phantom{a} & Definition (\ref{def:at}) \\

	\emph{IO Attributes} & \phantom{a} & $\glssymbol{io-attributes-summary} = \left\langle \mathcal{W}_{ioa}, \mathcal{B}_{ioa}, l_{\mathcal{W}_{ioa}} \right\rangle$ & \phantom{a} & Definition (\ref{def:ioa}) \\

	\emph{IO Attributes \& Types} & \phantom{a} & $\glssymbol{io-attributes-types-summary} = \left\langle \mathcal{W}_{ioat}, \mathcal{B}_{ioat}, l_{\mathcal{W}_{ioat}} \right\rangle$ & \phantom{a} & Definition (\ref{def:ioat}) \\
