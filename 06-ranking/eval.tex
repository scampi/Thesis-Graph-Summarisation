\section{Experiments}
\label{chap6:ranking:experiments}
...
\begin{description}
  ...
  \item[SS10] the ``Billion Triple Challenge''\footnote{Billion Triple Challenge: \url{http://vmlion25.deri.ie}} (BTC) dataset, containing more than one billion triples, with the assessments (a file containing relevance judgments on a sample of query answers) of the SemSearch2010\footnote{\label{chap6:ranking:dataset:ss10}SemSearch2010: \url{http://km.aifb.kit.edu/ws/semsearch10}} challenge;
  \item[SS11] the BTC dataset with the assessments of the ``Entity Search track'' of the SemSearch2011\footnote{\label{chap6:ranking:dataset:ss11}SemSearch2011: \url{http://km.aifb.kit.edu/ws/semsearch11/}} challenge.
\end{description}
...
\begin{enumerate}
	...
	\item analysed\footnote{The analysis of a text consists in splitting on a separator character such as whitespace, in order to produce tokens to be indexed. In the case of URIs, additional separators such as slashes `/' and dots `.' are also used. See \cite{McCandless:2010:LAS,buttcher:2016:information} for further details.} each description so that it may be retrieved based on a query that is pre-defined by the SemSearch\footnote{See footnotes \ref{chap6:ranking:dataset:ss10} and \ref{chap6:ranking:dataset:ss11}} challenges; and
...
%\subsection{Effectiveness of the \gls{MF} Ranking Model}
%\label{sec:mv-fields-effectiveness}
%
% Next we perform a comparison between the \gls{MF} extensions, i.e., BM25MF and PL2MF, against other ranking approaches.

\subsection{\emph{The Normalisation Parameters}}

In this section, we study the impact on the ranking of the normalisation parameters. In addition to the length normalisation of field-based ranking function (Normalisation~\ref{chap6:ranking:norm:content}), the \gls{MF} ranking model offers an additional normalisation on the attribute's cardinality (Normalisation~\ref{chap6:ranking:norm:degree}). The effectiveness of the PRF and DFR frameworks depends on finding the right values for the normalisation parameters. However, these parameters are highly dependent on the dataset.\\

Figures~\ref{chap6:ranking:fig:bm25mf-norm} and~\ref{chap6:ranking:fig:pl2mf-norm} depict the impact of the normalisation parameters on the ranking performance of \gls{BM25MF} and \gls{PL2MF}, respectively. For all three datasets, the figures depict on the \textbf{Y} axis the Mean Average Precision (MAP) scores, and on the \textbf{X} axis the Normalisation~\ref{chap6:ranking:norm:content}, i.e., with parameter $b_v$ in Figure~\ref{chap6:ranking:fig:bm25mf-norm} (resp., parameter $c_v$ in Figure~\ref{chap6:ranking:fig:pl2mf-norm}). Each curve plots the results with a fixed Normalisation~\ref{chap6:ranking:norm:degree} parameter, i.e., $b_a$ in Figure~\ref{chap6:ranking:fig:bm25mf-norm} and $c_a$ in Figure~\ref{chap6:ranking:fig:pl2mf-norm}.

The grid of parameters values in Figure~\ref{chap6:ranking:fig:bm25mf-norm} ranges from $0$ to $1$ with a step of $0.1$. In Figure~\ref{chap6:ranking:fig:pl2mf-norm}, the grid ranges from $0.5$ to $10.5$ with a step of $1$. Although these parameters can be attribute and value-specific, this experiment considers a constant parameter in order to reduce the number of combinations and to lower the variability of the results. Dashed lines depict the MAP scores of BM25F and PL2F and solid lines the scores of their \gls{MF} extension, \gls{BM25MF} and \gls{PL2MF} respectively.\\

These plots show that Normalisation~\ref{chap6:ranking:norm:degree}, the normalisation introduced with the \gls{MF} ranking model, improves the performance. Indeed, the Normalisation~\ref{chap6:ranking:norm:content} parameters $b_v$ and $c_v$, which are traditional to the field-based ranking model, do not grasp completely the heterogeneity in the data, which results in lower performance when compared to the \gls{MF} extensions. This indicates that the distinction between leaves within an attribute has a positive effect on the ranking.

\begin{figure}
	\centering
	\begin{subfigure}{\textwidth}
		\centering
		\input{06-ranking/figures/tikz/norm-exp-bm25mf}
	...
	\begin{subfigure}{\textwidth}
		\centering
		\input{06-ranking/figures/tikz/norm-exp-pl2mf}
		\addtocounter{subfigure}{-1}
		...
		\label{chap6:ranking:fig:pl2mf-norm}
	\end{subfigure}
...
\begin{labeling}{\emph{BM25~\cite{robertson:1994:sigir}}}
  ...
  \item[\emph{BM25~\cite{robertson:1994:sigir}}] considers the document as a simple bag of words. It is a function of the term frequency derived from a two-Poisson model and it uses an entity-length normalisation. The entity length is computed as the sum of the \emph{field length} defined in the Section~\ref{chap6:ranking:ranking-wod}.
...
\input{06-ranking/figures/experiments/norm-param}

Using such parameters, we report in Figures~\ref{fig:bm25mf-field-cmp-bar}~and~\ref{chap6:ranking:fig:pl2mf-field-cmp-bar} the performance of the ranking functions on the three datasets, for \gls{BM25MF} and \gls{PL2MF} respectively. The raw results of the bar charts are available in the Appendix section, in Table~\ref{tab:ranking-cmp-results}.
...
%We note that for field-based ranking models and their \gls{MF} extensions, the attribute label is considered as a value node, in order to be a source of potential relevant terms.
...
\input{06-ranking/figures/experiments/mf-field-cmp-bar}

\subsection{Effectiveness of the Weights}

In this section, we evaluate the impact of the presented weights over the \gls{MF} ranking extensions. First, we discuss the impact of discarding the attribute label as a source of possible relevant terms on the ranking performance. Then, we evaluate the weights from Section~\ref{chap6:ranking:weights} developed for the \gls{MF} model.
...
We provide for each dataset the best performing parameters described in Section~\ref{chap6:ranking:leaf-coverage} for the control function. For the three datasets regardless of the \gls{MF} extension, the values are as follows:
\begin{description}
...
\label{chap6:ranking:ael-weight-effect}
...
\begin{figure}
	...
	\begin{subfigure}{.483\textwidth}
		\centering
		\resizebox{\textwidth}{!}{
			\input{06-ranking/figures/experiments/cmp-w-wo-att}
		}
	...
	\begin{subfigure}{.483\textwidth}
		\centering
		\resizebox{\textwidth}{!}{
			\input{06-ranking/figures/experiments/cmp-qc}
		}
	...
	\begin{subfigure}{.483\textwidth}
		\centering
		\resizebox{\textwidth}{!}{
			\input{06-ranking/figures/experiments/cmp-lc}
		}
	...
	\begin{subfigure}{.483\textwidth}
		\centering
		\resizebox{\textwidth}{!}{
			\input{06-ranking/figures/experiments/cmp-ael}
		}
	...
	\begin{subfigure}{.8\textwidth}
		\centering
		\resizebox{\textwidth}{!}{
			\input{06-ranking/figures/experiments/cmp-all}
		}
...
\label{chap6:ranking:summary-ranking}
...
\subsection{Dataset}
\label{chap6:ranking:summary-ranking:dataset}
...
If the detail is either a URI such as \href{http://dbpedia.org/resource/Paris}{<http://dbpedia.org/resource/Paris>}, a literal such as \emph{"Paris"}, then it is replaced with a unique variable. In the case where a literal is a type (Section~\ref{chap2:semi-structured-data:type}), it is then kept as is.
...
Such a classification scheme allows to capture two aspects of a graph summary that have an impact on its \emph{precision} introduced in Section~\ref{chap5:precision:error-classification}. The size of a star in a query pertains to the attribute and type error. In addition, the number of stars in a query pertains to the \gls{connectivity} error.
...
Table~\ref{chap6:ranking:tab:usewod2013-queries} reports general statistics about the queries extracted from the USEWOD2013 DBpedia~3.3 logs using the function $qe$. The full list of queries is available in the Appendix~\ref{appendix:ranking}. For each category of complexity as defined in the previous section, we report in the table
\begin{inparaenum}[(a)]
...
\begin{table}
	...
	\label{chap6:ranking:tab:usewod2013-queries}
\end{table}
...
\begin{figure}
	...
	\resizebox{.6\textwidth}{!}{
		\input{06-ranking/figures/tikz/summary-ranking-ex.tex}
	}
	...
	\label{chap6:ranking:fig:sparql-solution}
\end{figure}

As an example, we consider the graph in Figure~\ref{chap6:ranking:fig:sparql-solution} that is a solution of the query from the previous section over a graph summary. A number between parenthesis indicates a statistic associated with the node. For instance, there is one node from the entity graph that got mapped to the sumnode $s_1$.
...
A straightforward approach consists in summing up all the statistics available to a subgraph that matched a query. The rationale is that the greater the sum, the more relevant is that solution. The result is $5$ in Figure~\ref{chap6:ranking:fig:sparql-solution}.

%\subparagraph{Field-based approach.}

%In this approach, we apply the Normalisation~\ref{chap6:ranking:norm:content}. The consequence in Figure~\ref{chap6:ranking:fig:sparql-solution} is that the \emph{average cardinality} of the attribute ``dbp:owner'' is taken into consideration.

\subparagraph{MF-based approach.}

In this approach, we apply both normalisations available with the \gls{MF} ranking model, i.e., Normalisations~\ref{chap6:ranking:norm:content} and~\ref{chap6:ranking:norm:degree}. The difference of this approach with previous is that the \emph{average cardinality} of every node is considered.

\subsection{Evaluation}
\label{chap6:ranking:summary-ranking:eval}

In Section~\ref{chap6:ranking:summary-ranking:dataset} we presented a dataset of queries that were extracted from logs of the DBpedia SPARQL endpoint. In Section~\ref{chap6:ranking:summary-ranking:eval} we introduced two ranking functions that we apply on sub-graphs of a graph summary which is, in this evaluation, that of DBpedia.
...
\begin{enumerate}
	\item whether a solution that provides bindings for the \emph{?POF} variable that are part of the error set (Section~\ref{chap5:precision:error-classification}) is ranked lower that one that does not; and
...
Figures~\ref{chap6:ranking:fig:summary-ranking-norm} depict the impact of the Normalisation~\ref{chap6:ranking:norm:content} and~\ref{chap6:ranking:norm:degree} parameters on the ranking of graphs. On the X axis we vary the value of the parameter $b_a$ for Normalisation~\ref{chap6:ranking:norm:degree}. Each curve corresponds to a value of the parameter $b_v$ fro Normalisation~\ref{chap6:ranking:norm:content}. On the Y axis we report the MAP measurement for the \gls{MF}-based ranking approach. The raw results of the curves are available in Appendix~\ref{appendix:ranking:tab:summary-ranking-norm}.\\
...
Figure~\ref{chap6:ranking:fig:summary-ranking-norm-simple} depicts the \emph{average} of the MAP measurements from the seven queries classes of the simple category. Figure~\ref{chap6:ranking:fig:summary-ranking-norm-complex} depicts the \emph{average} of the MAP measurements from the ten queries classes within the complex category.\\

We observe that with simple graphs in Figure~\ref{chap6:ranking:fig:summary-ranking-norm-simple} the parameter $b_a$ for Normalisation~\ref{chap6:ranking:norm:degree} reported on the X axis has little effect on the ranking. On the contrary, we see it has a positive impact in Figure~\ref{chap6:ranking:fig:summary-ranking-norm-complex} for complex graphs.
The parameter $b_v$ for Normalisation~\ref{chap6:ranking:norm:content} increases the performance of the ranking as its value reaches $1$.
...
\input{06-ranking/figures/summary-group-norm}
...
Table~\ref{chap6:ranking:tab:summary-ranking-cmp} reports the MAP values of the basic and \gls{MF}-based approaches over the \gls{typessummary} of Dbpedia. We write the MAP value in bold for the approach that has the greatest. We observe that for a majority of the query complexities, the \gls{MF}-based approach provides a better ranking than the basic approach.

This shows the benefits of the \gls{MF} ranking model of normalising the data over two levels: the Normalisation~\ref{chap6:ranking:norm:content} allows to compare several solutions to a query regardless of different statistics they might have; with the Normalisation~\ref{chap6:ranking:norm:degree}, it is possible to rank solutions to a query that have different degree of a same attribute.

\input{06-ranking/figures/summary-ranking-comp}
...
%\begin{figure}
	...
	%\resizebox{.7\textwidth}{!}{
		...
		%\end{tikzpicture}
	%}
	%\caption{Comparison of the basic and \gls{MF}-based ranking approaches on the Types summary of DBpedia 3.3}
%\end{figure}
